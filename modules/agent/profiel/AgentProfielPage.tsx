...
// Fix: Removed unused and incorrectly imported Employee type from store.ts re-export. 
// Types should ideally be imported from types.ts if needed, but Employee is not used here.
...
export const AgentProfielPage: React.FC = () => {
  ...
  // Waterproof Lock Ref
  const savingPersonalRef = useRef(false);
  ...
  const handlePersonalSubmit = () => {
      // Waterproof double-click lock
      if (savingPersonalRef.current) return;
      savingPersonalRef.current = true;

      if (!personalData.firstName || !personalData.lastName || !personalData.address || !personalData.phone) {
          alert('Vul alle verplichte velden in.');
          savingPersonalRef.current = false;
          return;
      }
